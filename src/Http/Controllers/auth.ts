...
import { LoginProps } from '../Model/Auth/AuthTransformer';
...
import AuthTransformer from '../Model/Auth/AuthTransformer';
...
const login = async( req: Request , res: Response, next: NextFunction ) => {
    ...
    try {
        ...
        const user = await userService.getUser(UserCriteria.createByUsername(body.username));
        ...
        if(err instanceof UserNotFoundException){
            err = new BadCredentialsException('Username or password incorrect');
        }
        next(err);
    }
}
